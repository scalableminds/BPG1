...
class FilemakerAPI(con: java.sql.Connection) {
  ...
  def extractStudents(): List[UserLike] = {
    ...
    val l = scala.collection.mutable.ListBuffer[UserLike]()
  
    while (rs.next) {
    if(extractTags(rs).exists(tag => tag.contains("Student")))
      l.append(UserLike(rs.getString("Vorname"), rs.getString("Name"), rs.getString("email")))
    }
    l.toList
  }
